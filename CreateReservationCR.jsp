...
<body>
	<%
		String flightNum = request.getParameter("crfnum");
		System.out.println("fuq"+flightNum);
		String username = request.getParameter("cruser");
		String password = request.getParameter("crpassword");
	
		System.out.println("user "+username+" pwr "+password);
		...
		String class_info = request.getParameter("crclass");
			...
			while(result.next()){
				if(result.getInt("cnt")==1){
					...
					while(result.next()){
						...
						String stm = "Insert into reservations(ticket_number, t_num, purchase_date, departure_time, arrival_time, special_meal, class, flight_type, connecting_flight, booking_fee, total_fare, username, password, flight_number, first_name, last_name, airline_id, start_date, end_date, waitlist)"
								+ "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
						...
						ps.executeUpdate();
						//result = stmt.executeQuery("Select * from reservations where flight_number=1 and username='msaleh'");
						//result.next();
						//System.out.println(result.getInt("ticket_number"));
